...
class DecorateActionSpecialObject(sphinx.directives.ObjectDescription[str]):
	doc_field_types = [
	...
	option_spec = {
		**sphinx.directives.ObjectDescription[str].option_spec,
		'num': docutils.parsers.rst.directives.unchanged,
	}

	def add_target_and_index(self, name: str, sig: str, signode: docutils.nodes.TextElement):
		target = f'special:{identifier_to_target(name)}'
		...
		domain.note_object('special', name, self.env.docname, target)

		self.indexnode['entries'].append(('single', f'Action specials; {name}', target, '', None))

	def transform_content(self, content_node: sphinx.addnodes.desc_content):
		param_list = sphinx.addnodes.desc_parameterlist()
		for field in content_node.findall(docutils.nodes.field_name):
			_, param_name = field.astext().split(None, maxsplit=1)
			param_node = sphinx.addnodes.desc_parameter('', param_name)
			param_list.append(param_node)

		signode = content_node.parent.next_node(condition=sphinx.addnodes.desc_signature)
		signode.append(param_list)

	def handle_signature(self, sig: str, signode: docutils.nodes.TextElement) -> str:
		if num := self.options.get('num', None):
			signode.append(sphinx.addnodes.desc_sig_element(num, f'{num}:'))
...
class DecorateConstantObject(sphinx.directives.ObjectDescription[str]):
	...
	def handle_signature(self, sig: str, signode: docutils.nodes.TextElement) -> str:
		...
		if value := self.options.get('value', None):
			signode.append(sphinx.addnodes.desc_sig_element(value, f'({value})'))
		return sig
...
class DecorateDomain(sphinx.domains.Domain):
	...
	directives = {
		...
		'special': DecorateActionSpecialObject,
	}
